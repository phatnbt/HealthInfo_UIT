...
This is a repair/sensitivity release for review, **not an approved final submission**. Historical Day22 results and their 30-entry freeze manifest are unchanged. Historical XGBoost reproduction remains OPEN: the verified Windows wheel differs by up to 0.0181306514. LR/RF pass the locked 1e-8 tolerance. Artifact-integrity PASS does not mean that every model has been retrained successfully.
...
`scripts/mi_explainability.py` and `scripts/day14_16_shap_explainability_v2.py` use exact native TreeSHAP on a CSR DMatrix and check that contributions sum to the native raw margin. Across the 120 MI explanation arms, maximum additivity residual was **7.1526e-6**, below 3e-5 (float32 tree arithmetic); LR/RF residuals were at machine precision. Historical XGBoost SHAP figures/tables are quarantined for primary claims until sparse-corrected explanations of a reproducible estimator are available. New current-runtime explanations remain sensitivity artifacts and do not overwrite frozen primary results.
...
- Original runtime/model/predictions needed to resolve historical XGB; replacing historical numbers requires an explicitly versioned scientific decision.
