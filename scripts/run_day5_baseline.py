...
def main():
 import argparse
 parser=argparse.ArgumentParser(description='Locked Day5 baseline; person-level exports require an explicit flag.')
 parser.add_argument('--data-dir',type=Path,required=True)
 parser.add_argument('--out-dir',type=Path,required=True)
 parser.add_argument('--export-person-level',action='store_true',help='Private output only: includes HHX, labels and probabilities.')
 args=parser.parse_args()
 BASE=args.data_dir.resolve(); OUT=args.out_dir.resolve()
 for fn in ['analysis_ready_MEDNG_FINAL_FEATURELOCK_RAWCODES.csv','analysis_ready_MEDDL_FINAL_FEATURELOCK_RAWCODES.csv']:
  if not (BASE/fn).is_file(): parser.error('Missing input: '+str(BASE/fn))
 if BASE==OUT: parser.error('Use a separate output directory.')
