...
def main():
    ...
    parser.add_argument("--allow-historical-drift", action="store_true", help="Generate separate current-runtime sensitivity explanations; never replace historical primary.")
    ...
    for outcome_index, (outcome, spec) in enumerate(OUTCOMES.items()):
        ...
        for model_index, model_name in enumerate(MODELS):
            ...
            if status != "PASS" and not args.allow_historical_drift:
            ...
            if model_name == "LR":
                ...
                if model_name == "XGBoost":
                    import xgboost
                    matrix=xgboost.DMatrix(x_test_sparse[explain_indices])
                    native=model.get_booster().predict(matrix,pred_contribs=True,approx_contribs=False)
                    margin=model.get_booster().predict(matrix,output_margin=True)
                    residual=float(np.max(np.abs(native.sum(axis=1)-margin)))
                    if residual>3e-5: raise RuntimeError(f"Native TreeSHAP additivity: {residual}")
                    explanation=shap.Explanation(values=native[:,:-1],base_values=native[:,-1],data=x_test)
                    audit_rows[-1]["SHAP_additivity_max_residual"]=residual
                    audit_rows[-1]["Sparse_dense_probability_max_difference"]=float(np.max(np.abs(model.predict_proba(x_test_sparse[explain_indices])[:,1]-model.predict_proba(x_test)[:,1])))
                else:
            ...
            if model_name in TREE_MODELS and args.interaction_n > 0:
                ...
                interactions = normalize_interaction_output(
                    (model.get_booster().predict(xgboost.DMatrix(x_test_sparse[explain_indices[interaction_indices]]),pred_interactions=True)[:,:-1,:-1]
                     if model_name=="XGBoost" else explainer.shap_interaction_values(x_test[interaction_indices])), model_name
                )
    ...
    config = {
        ...
        "explainers": {
            ...
            "XGBoost": "Exact native TreeSHAP on CSR DMatrix; raw margin; additivity checked; implicit missing values preserved.",
        },
